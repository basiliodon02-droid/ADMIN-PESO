  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    function renderTable(searchWords = []) {
      ...
      for (const [id, d] of payloadStore) {
        ...
        if (searchWords.length && !searchWords.every(w => searchableText.includes(w))) continue;
      ...
      try {
        ...
        for (const v of cache.vacancies) {
          ...
          for (const [uid, a] of Object.entries(cache.applicant)) {
            ...
            let score = 0;
            const matchedWords = [];
            weightedWords.forEach(w => { if (text.includes(w.word)) { score += w.weight; matchedWords.push(w); } });
            if (score > 0) {
