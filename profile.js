...
openBtn.onclick = async () => {
  ...
  if (result.success == true) {
    ...
    document.getElementById("contactCell").innerText = result.data[0].contact_number ?? '-';
    document.getElementById("roleCell").innerText = result.data[0].role;
    document.getElementById("createDateCell").innerText = result.data[0].created_at;
    ...
    document.getElementById("editContact").value = result.data[0].contact_number ?? '';
...
form.addEventListener('submit', async (e) => {
  ...
  const result = await editProfile(userId, firstName, middleName, lastName, suffix, email, contact_number, password);
  if (result.success == true) {
...
async function editProfile(user_id, firstName, middleName, lastName, suffix, email, contact_number, password) {
  const { error } = await supabase
    .from("Users")
    .update({ firstName: firstName, middleName: middleName, lastName: lastName, suffix: suffix, email: email, contact_number: contact_number, password: password })
