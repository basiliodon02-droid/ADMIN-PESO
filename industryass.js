(() => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function loadStore() {
      const industries =
        JSON.parse(localStorage.getItem(STORE_KEYS.industries)) ||
        defaultData.industries;
      const jobs =
        JSON.parse(localStorage.getItem(STORE_KEYS.jobs)) || defaultData.jobs;
      const assigns =
        JSON.parse(localStorage.getItem(STORE_KEYS.assigns)) ||
        defaultData.assigns;
      const idCounters =
        JSON.parse(localStorage.getItem(STORE_KEYS.idCounters)) ||
        defaultData.idCounters;
    ...
    const modalIndustry = window.bootstrap
      ? new bootstrap.Modal(byId("modalIndustry"))
      : null;
    ...
    byId("btnAddIndustry")?.addEventListener("click", () => {
      ...
      byId("modalIndustry").querySelector(".modal-title").textContent =
        "Add Industry";
      modalIndustry?.show();
    });
    ...
    byId("tblIndustries")?.addEventListener("click", async (e) => {
      ...
      if (action === "del") {
        if (
          !confirm(
            "Delete this Industry? Related jobs/assignments may still depend on this Industry."
          )
        )
          return;
    ...
    byId("formJob")?.addEventListener("submit", async (e) => {
      ...
      if (job_id) {
        ...
        if (checkIndustryJobResult.data.length > 0) {
          alert(
            "Industry is already assigned to another Job Role. Please choose a different Industry."
          );
          return;
        } else {
          //proceed
    ...
    function openAssignModal(editId = null) {
      ...
      if (editId) {
        const rec = fetchedIndustryJobAssignments.find(
          (a) => a.job_assignment_id == editId
        );
        if (!rec) return;
    ...
    byId("openAssignModalBtn")?.addEventListener("click", () =>
      openAssignModal()
    );
    ...
    byId("formAssign")?.addEventListener("submit", async (e) => {
      ...
      const [jobRoleId, jobTitle, jobIndustryId, jobIndustryName] =
        byId("assignJob").value.split("/");
      const [industryId, industryName] =
        byId("assignIndustry").value.split("/");
      ...
      if (idVal) {
        ...
        const dup = fetchedIndustryJobAssignments.some(
          (a) =>
            a.industry_id == industryId &&
            a.job_role_id == jobRoleId &&
            a.job_assignment_id != idVal
        );
        if (dup) {
        ...
        const result = await editIndustryJobAssignment(
          idVal,
          industryId,
          industryName,
          jobRoleId,
        ...
        //check for duplicate industry-job assignment
        const dup = fetchedIndustryJobAssignments.some(
          (a) => a.industry_id == industryId && a.job_role_id == jobRoleId
        );
        if (dup) {
          alert("This Industry ⇄ Job link already exists.");
          return;
        ...
        const result = await addIndustryJobAssignment(
          industryId,
          industryName,
          jobRoleId,
          jobTitle,
