(() => {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function onDateChange() {
      ...
      const statsIds = ["workerStats", "employerStats", "vacanciesStats", "applicationsStats"];
      const loaderIds = ["workerLoader", "employerLoader", "vacanciesLoader", "applicationsLoader"];
      ...
      document.getElementById("workers").innerText = workerResults.success ? workerResults.activeCount : 0;
      document.getElementById("workers-inactive").innerText = workerResults.success ? workerResults.totalCount - workerResults.activeCount : 0;
      ...
      document.getElementById("employers").innerText = employerResults.success ? employerResults.activeCount : 0;
      document.getElementById("employers-inactive").innerText = employerResults.success ? employerResults.totalCount - employerResults.activeCount : 0;
      ...
      document.getElementById("postedVacancies").innerText = vacancyResults.success ? vacancyResults.activeCount : 0;
      document.getElementById("closedVacancies").innerText = vacancyResults.success ? vacancyResults.closedCount : 0;
      ...
      document.getElementById("applicationsSubmitted").innerText = applicantResults.success ? applicantResults.totalCount : 0;
      document.getElementById("applicationsHired").innerText = applicantResults.success ? applicantResults.hiredCount : 0;
    ...
    window.onload = async () => {
      ...
      // Second row stats
      // -----------------------
      const [vacancyList, establishmentList, applicantList, pendingUsersList] = await Promise.all([
        getVacancyList(), getEstablishmentList(), getApplicantList(), getPendingUsersList()
      ]);
  ...
  async function getWorkersListByDate(dateFrom, dateTo) {
    return safeQuery(async () => {
      let query = supabase.from("Users").select("*").eq("role", "Worker").order("user_id", { ascending: true });
      ...
      return { success: true, data: workerList, totalCount, activeCount, pendingCount, deletedCount };
    });
  }

  async function getEmployerListByDate(dateFrom, dateTo) {
    return safeQuery(async () => {
      let query = supabase.from("Users").select("*").eq("role", "Employer").order("user_id", { ascending: true });
      if (dateFrom) query = query.gte("created_at", dateFrom);
      if (dateTo) query = query.lte("created_at", dateTo);
      const { data, error } = await query;
      if (error) throw error;
  ...
  async function getVacanciesListByDate(dateFrom, dateTo) {
    return safeQuery(async () => {
      let query = supabase.from("JobVacancy").select("*").order("vacancy_id", { ascending: true });
  ...
  async function getApplicantListByDate(dateFrom, dateTo) {
    return safeQuery(async () => {
      let query = supabase.from("JobApplication").select("*").order("application_id", { ascending: true });
      ...
      const totalCount = data.length;
      const hiredCount = data.filter(a => a.applicationStatus === "Hired").length;
      const notHiredCount = data.filter(a => a.applicationStatus !== "Hired").length;
      const forReviewList = data.filter(a => a.applicationStatus === "For Review");

      return { success: true, data: forReviewList, totalCount, hiredCount, notHiredCount };
    });
  }

  async function getVacancyList() {
    return safeQuery(async () => {
      const { data, error } = await supabase.from("JobVacancy").select("*").eq("status", "Active").order("vacancy_id", { ascending: true });
      if (error) throw error;
  ...
  async function getEstablishmentList() {
    return safeQuery(async () => {
      const { data, error } = await supabase.from("Establishment").select("*").eq("status", "Active").order("establishment_id", { ascending: true });
      if (error) throw error;
      return { success: true, data };
    });
  }
  ...
  async function getApplicantList() {
    return safeQuery(async () => {
      const { data, error } = await supabase.from("JobApplication").select("*").eq("applicationStatus", "For Review").order("application_id", { ascending: true });
      if (error) throw error;
      return { success: true, data };
    });
  }
  ...
  async function getPendingUsersList() {
    return safeQuery(async () => {
      const { data, error } = await supabase.from("Users").select("*").eq("status", "Pending").order("user_id", { ascending: true });
      if (error) throw error;
      return { success: true, data };
    });
  }
